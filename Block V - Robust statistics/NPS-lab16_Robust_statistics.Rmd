...
explicit_est_x_9 <- c(mean(x_9),
    mean(x_9, trim = .1), #trim argument allow us to perform trimmed mean -> with mean(x) we have trim=0
...
#increasing k(par) we will have less robust estimator -> decresing k will be more strong because we downweight points far away
```
...
function for robust fitting (#\_ nls is the non-robust version) .
Honestly, I do not find it straightforward to use, so we move directly
to robust multivariate estimators.
...
```{r}
dplyr::near(fit_MCD$raw.center,colMeans(Animals2[ind_best_subset,])) #I compute colmean of black points
...
#this is the reweightd estimate -> because reducing the sample we are loosing maybe the true variability
# -> I'm maintaining the old breakdown point acter the reweighting
...
## Robust regression
#_now lets look how to compute least trimmed square and least ..(?)
### A simple example
...
#_ ltsReg is more efficient and much more employed -> USE IT!
```
...
```{r}
#_ lms does not have plotting facilities
plot(fit_lts)

#_ from the last graph we can see that there are no vertical outliers; 14 is a good leverage point and the others are bad leverage points
```
